...
! # From the basic clawpack routine step1, rp is called with ql = qr = q.
	...
	double precision :: hl, ul, vl, hr, ur, vr, hbar, uhat, vhat, chat, n_1, n_2
    ...
    do i=2-mbc,nbrCells+mbc
		...
		fluxDiff(2) = (hr*ur*(ur*n_1 + vr*n_2) + 0.5*grav*hr**2) - (hl*ul*(ul*n_1 + vl*n_2) + 0.5*grav*hl**2)
		
		fluxDiff(3) = (hr*vr*(ur*n_1 + vr*n_2) + 0.5*grav*hr**2) - (hl*vl*(ul*n_1 + vl*n_2) + 0.5*grav*hl**2)
