...
! # From the basic clawpack routine step1, rp is called with ql = qr = q.
    ...
    do i=2-mbc,mx(1)+mbc
		...
		! # Flux differences
		fluxDiff(1) = auxl(i,1)*(hr*ur) - auxr(i-1,1)*(hl*ul) !+ hbar*uhat/auxl(i,1)*auxl(i,2)
		fluxDiff(2) = auxl(i,1)*(hr*ur*ur + 0.5*grav*hr*hr) - auxr(i-1,1)*(hl*ul*ul + 0.5*grav*hl*hl) - 0.5*grav*hbar*hbar*auxl(i,4)
		
		!write(*,*) fluxDiff(1), fluxDiff(2)
