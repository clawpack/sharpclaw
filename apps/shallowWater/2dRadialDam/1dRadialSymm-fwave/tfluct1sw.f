...
c        # u-c in left state (cell i-1)
         s0 = ql(i,2)/ql(i,1) - dsqrt(grav*ql(i,1))
...
c        # u-c to right of 1-wave
         hr1  = ql(i,1) + wave(i,1,1) 
         uhr1 = ql(i,2) + wave(i,2,1)
...
c        # u+c in right state  (cell i)
         s3 = qr(i,2)/qr(i,1) + dsqrt(grav*qr(i,1))
              
c        # u+c to left of 2-wave
         hl2  = qr(i,1) - wave(i,1,2) 
         uhl2 = qr(i,2) - wave(i,2,2)
...
c      # Compute the total fluctuation
               adq(i,m) = amdq(i,m) + apdq(i,m)
  220       continue
