...
c
      dimension wave(1-mbc:mx+mbc, meqn, mwaves)
      dimension    s(1-mbc:mx+mbc, mwaves)
      dimension   ql(1-mbc:mx+mbc, meqn)
      dimension   qr(1-mbc:mx+mbc, meqn)
      dimension  apdq(1-mbc:mx+mbc, meqn)
      dimension  amdq(1-mbc:mx+mbc, meqn)
      dimension  adq(1-mbc:mx+mbc, meqn)
c
...
c     ------------
      ...
      dimension delta(3)
      logical efix
      common /comrp/  g    !# gravitational parameter 
...
c
        do 10 i = 2-mbc, mx+mbc
         h(i) = (ql(i,1)+qr(i,1))*0.50d0
         hsqrtl = dsqrt(ql(i,1))
         hsqrtr = dsqrt(qr(i,1))
         hsq2 = hsqrtl + hsqrtr
         u(i) = (ql(i,mu)/hsqrtl + qr(i,mu)/hsqrtr) / hsq2
         v(i) = (ql(i,mv)/hsqrtl + qr(i,mv)/hsqrtr) / hsq2
         a(i) = dsqrt(g*h(i))
   10    continue
c
c
...
c     # find a1 thru a3, the coefficients of the 3 eigenvectors:
      do 20 i = 2-mbc, mx+mbc
         ...
         delta(2) = qr(i,mu) - ql(i,mu)
         delta(3) = qr(i,mv) - ql(i,mv)
...
c
            h1 = ql(i,1)+wave(i,1,1)
            hu1= ql(i,mu)+wave(i,mu,1)
...
c
            hi = qr(i,1)
            s03 = qr(i,mu)/hi + dsqrt(g*hi)
            h3=qr(i,1)-wave(i,1,3)
            hu3=qr(i,mu)-wave(i,mu,3)
