...
c     # with variable speed limit umax, using the f-wave approach
c     # umax is stored in aux(i,1)
c
c
...
c        ###############################################
c
c        # Flux in each cell and flux difference:
         fim1 = auxl(i-1,1)*qr(i-1,1)*(1.d0 - qr(i-1,1))
         fi = auxl(i,1)*ql(i,1)*(1.d0 - ql(i,1))
         fwave(i,1,1) = fi - fim1

c        # Characteristic speed in each cell:
