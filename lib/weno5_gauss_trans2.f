c
c ===================================================================
       subroutine weno5_gauss_trans2(ixy,maxm,meqn,mbc,mx,q1d,q_gauss,
     2                          auxl,auxr,w,w5)
c ===================================================================
...
c  slice direction
      ...
      dimension gg(1-mbcweno:mxweno+mbcweno,meqnweno)
...
c       # 'm'th characteristic field
      do 99 m=1,meqn
        do i=2-mbc,mx+mbc
          gg(i,m)=0.d0
          do mm=1,meqn
            gg(i,m)=gg(i,m)+evl(i,m,mm)*dq(i,mm)
          enddo
        enddo
  99  continue
...
c       # 'm'th characteristic field
        ...
        do  i = 2-mbc,mx+mbc
           do m1 = -2,2
              hh(i,m1) = 0.d0
              do mm=1,meqn 
                hh(i,m1) = hh(i,m1)+ evl(i,m,mm)*
     &            gg(i+m1,m)*evr(i+m1,mm,m)
              enddo !meqn loop
           enddo !m1 loop
